...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    /* Input parameters:
        ...
        Return:
        - estimaated similarity for the given region using point similarity principle (and linear intensity interpolation!!!!)
    */
    if (nrhs!=2) 
      ...
      for (int px=rx;px<rX;px++){
        ...
        if (roiRdef || maskR){
            ...
            if (maskR) {
                ...
                int YR=floor(y+0.5); 
                int ZR=floor(z+0.5);                 
                //pMaskR = pMaskR0 + X1 +NXR*( Y1 + NYR * Z1 ); // zaokroženo navzdol!!!!! TODO floor(x+0.5)...
